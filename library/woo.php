...
add_action( 'woocommerce_account_club_endpoint', function () {
	...
	echo '<p class="luvit-drops__sub">'
		...
		. '، وحدة فيهن بتيجي معها هدية مننا.'
		. '</p>';
	...
	/* 🔴 **ولا وعد بصنف ولا بقيمة** · صاحب العلامة هو اللي بيقرّر شو
	   الهدية، وأي تحديد هون بيصير وعداً لازم حدا يوفيه. */
	echo '<p class="luvit-drops__sub">'
		. 'وشو الهدية؟ منختارها إحنا وبتختلف من مرة لمرة · بس بتكون إشي '
		. 'تستاهل، مش عيّنة صغيرة تنرمى.'
		. '</p>';
...
/* ══════════════════════════════════════════════════════════════════════
   💧 LUVIT_DOSE · «قبل أول استعمال» بإيميل الطلب المُسلَّم · ٧ أيلول
   ══════════════════════════════════════════════════════════════════════
   ريّان وافق على الحركة، والفكرة إنها **الرافعة الوحيدة اللي بتقصّر فترة
   إعادة الشراء بإنها تخلّي المنتج يشتغل أحسن، لا بإنها تبيع إشي**.

   ── ليش هالإيميل بالذات ─────────────────────────────────────────────
   بينطلق من «customer_completed_order»، والحالة «completed» بتنكتب لما
   المندوب يضغط «تسلّمت» فبيسقّطها luvit_d_project(). يعني **الكرتونة
   بإيدها فعلاً** وقت ما تقرا. ولا إرسال زيادة، ولا خطر وصول جديد.

   🔴 **ومش على صفحة التأكيد** · هناك لسا ما استلمت، فنصيحة جرعة قبل ما
      توصلها البضاعة بتنقرا حشواً. صفحة التأكيد إلها بندها المستقل.

   ── ولا رقم ولا تردّد بينكتب إلا وهو مثبت ───────────────────────────
   🔴 التحليل ادّعى إنّ L102 «مرّتين باليوم». **وصف المنتج ما بيقول هيك**
      (بيقول «ترطيب يدوم طول اليوم»). فما انكتب تردّد لولا منتج إلا اللي
      **مكتوب بوصفه**: L106 وصفه حرفياً «تقشير مش يومي».
      [[ingredient-percentages-are-real]] · [[prove-dont-claim]]

   ⤷ والأرقام اللي انكتبت:
     · أربع نقاط للسيروم ≈ ٠٫٢١ غرام · متوسّط مقيس (Yamaguchi 2017 · ٢٦٥ سيدة)
     · حبة حمص للكريم ≈ ٠٫٢٤ غرام · نفس الدراسة
     · إصبعان للواقي = الجرعة اللي بتوصل ٢ ملغم/سم٢ اللي بينقاس عليها SPF
       (Taylor & Diffey · BMJ 2002;324:1526)

   ⚠️ **وسطر «العبوة بتخلص أسرع» مقصود وإلزامي.** بدونه هاي نصيحة بترفع
      الاستهلاك مغلّفة كخدمة. مع السطر بتصير صدقاً كامل الشفافية.

   ── والدعم ──────────────────────────────────────────────────────────
   بالرد على نفس الرسالة · **مش برقم واتساب**، لأنّ الرقم لسا ما تأكّد
   إنّ في مين بيرد عليه. بينضاف ببند مراجعة الإيميلات لما يتأكد ريّان.
   ══════════════════════════════════════════════════════════════════════ */
function luvit_dose_skus( $order ) {
	$out = array();
	foreach ( $order->get_items() as $item ) {
		...
		/* 🔴 **قطع الباقة بتنقرا من luvit_pk_map() لا ببحث بالعنوان.**
		   get_page_by_title() مهملة من ووردبريس ٦٫٢ والموقع على ٧٫١،
		   والمحلّل عندنا جاهز ومفحوص وبيرجّع المعرّفات محلولة أصلاً. */
		$pid = $item->get_product_id();
		if ( $pid && function_exists( 'luvit_pk_map' ) ) {
			$map = luvit_pk_map();
			foreach ( array_merge( $map['routines'], $map['duos'] ) as $b ) {
				if ( (int) $b['id'] !== (int) $pid || empty( $b['resolved'] ) ) {
					continue;
				}
				foreach ( (array) $b['ids'] as $cid ) {
...
function luvit_dose_html( $order ) {
	if ( ! $order || ! is_a( $order, 'WC_Order' ) ) {
		return '';
	}
	$skus = luvit_dose_skus( $order );
	if ( ! $skus ) {
		return '';
	}
	$has = function ( $s ) use ( $skus ) {
		return in_array( $s, $skus, true );
	};

	$name = trim( (string) $order->get_billing_first_name() );
	/* [[empty-values-reach-the-page-as-text]] · واسم لاتيني بينقرا غريب بجملة عربية */
	$hi = ( '' !== $name && ! preg_match( '/[A-Za-z]/', $name ) ) ? 'أهلين ' . esc_html( $name ) . '،' : 'أهلين،';

	$li = array();
	$li[] = '<strong>الترتيب:</strong> غسول، بعدين تونر، بعدين سيروم، بعدين كريم. من الأخفّ للأتقل.';
	$li[] = '<strong>الكمية:</strong> السيروم أربع نقاط مش نقطة، والكريم حبّة بحجم حبّة الحمّص مش مسحة. '
		/* «نصّ» مذكّر · «ما بتعطي» كانت بتعامله كمؤنّث */
		. 'نصّ الكمية ما بيعطي نصّ النتيجة.';

	if ( $has( 'L106' ) ) {
		$li[] = '<strong>سيروم التقشير:</strong> مرّتين لثلاث مرّات بالأسبوع، مش كل يوم. '
			. 'وهاد مكتوب على وصفه: «تقشير مش يومي».';
	}
	if ( $has( 'L106' ) || $has( 'L105' ) ) {
		$li[] = '<strong>ومع الأحماض:</strong> الأحماض بتخلّي البشرة أحسّ للشمس، '
			. 'فواقي الشمس بالنهار مش اختيار معها.';
	}
	if ( $has( 'L101' ) ) {
		$li[] = '<strong>سيروم فيتامين سي:</strong> أغلب الناس بتحطّه بالصبح، '
			. 'وإذا حابّة تحطّيه مسا كمان ما في مانع.';
	}
	if ( $has( 'L112' ) ) {
		$li[] = '<strong>واقي الشمس:</strong> آخر خطوة بالصبح. والكمية الصح طول إصبعين، '
			. 'السبّابة والوسطى، من أول الإصبع لآخره. وأقلّ من هيك، حمايتك بتصير أقلّ بكتير '
			. 'من الرقم اللي على العبوة.<br>وصح، بالكمية الصح العبوة بتخلص أسرع. هاد طبيعي، '
			. 'وهاد بالضبط الفرق بين حماية حقيقية وحماية على الورق.';
	}

	$o  = '<div style="margin:28px 0 8px;padding:20px 22px;border-radius:14px;'
		. 'background:#F2FAFC;border:1px solid #D6EDF3;direction:rtl;text-align:right;">';
	$o .= '<p style="margin:0 0 6px;font-size:17px;font-weight:700;color:#1A2529;">قبل أول استعمال</p>';
	/* 🔴 **ولا رقم مكتوب بالإيد فوق قائمة متغيّرة.**
	   كان فوق القائمة عدد مكتوب بالإيد، والقائمة **شرطية**: بندان
	   أساسيان وباقيها حسب اللي بالطلب. فطلب بلا واقي ولا أحماض ولا
	   فيتامين سي بيطلع فيه **بندان** والنصّ بيوعد بأكثر · وطلب كامل
	   بيطلع فيه خمسة.
	   ⤷ بلّغ عنها ريّان من الإيميل الحقيقي ٧ أيلول، وصادف إنّ طلب ٥٩٧
	     كان تلاتة فعلاً فما بانت. **الصدفة مش فحص.**
	   ⤷ والحل شيل الرقم لا حسابه · العدد ما بيضيف ولا إشي للزبونة،
	     وأي حساب بيرجع يفتح باب صيغة العدد بالعربي بلا مكسب.
	   [[empty-values-reach-the-page-as-text]] */
	$o .= '<p style="margin:0 0 14px;font-size:14px;color:#41565C;">' . $hi
		. ' وصلِك طلبك. وفي إشيات بسيطة بتفرّق أكتر ممّا بتتوقّعي.</p>';
	$o .= '<ul style="margin:0;padding-inline-start:18px;font-size:14px;line-height:1.9;color:#1A2529;">';
	foreach ( $li as $line ) {
		$o .= '<li style="margin-bottom:8px;">' . $line . '</li>';
	}
	$o .= '</ul>';
	$o .= '<p style="margin:14px 0 0;font-size:13px;color:#41565C;">'
		. 'وإذا صار حرقان بيضلّ بعد ما تشطّفي، أو احمرار ما بيروح، وقّفي وردّي على هالرسالة.'
		. '</p>';
	$o .= '</div>';
...
add_action( 'woocommerce_email_order_details', function ( $order, $sent_to_admin, $plain_text, $email ) {  // LUVIT_DOSE
	if ( $sent_to_admin || $plain_text ) {
		return;
	}
	if ( ! $email || ! isset( $email->id ) || 'customer_completed_order' !== $email->id ) {
		return;
	}
	echo wp_kses_post( luvit_dose_html( $order ) );
}, 15, 4 );

/* ══════════════════════════════════════════════════════════════════════
   🎁 LUVIT_CLUB_FLAG · الطلبية المستحقّة بتتعلّم بملاحظة إدارية
   ══════════════════════════════════════════════════════════════════════
   🔴 **بلا هالكتلة الوعد بينكسر بصمت.** الهدية بتتحط بالكرتونة **بإيد
      بني آدم**، وما في مكان بيقوله إنّ هالطلبية هي المستحقّة. فالزبونة
      بتقرا «هديتك مع الطلبية رقم ١٠» وبيوصلها كرتون عادي.
      [[build-it-before-there-is-data]]

   ⤷ والملاحظة بتظهر بشاشة الطلب **وبإيميل الطلب الإداري** سوا، وهاد
     المسار اللي بيمرق منه صاحب العلامة فعلاً.

   ⚠️ **وبتنكتب مرة وحدة** · محروسة بميتا عشان إعادة الحفظ ما تكرّرها.
   ══════════════════════════════════════════════════════════════════════ */
add_action( 'woocommerce_checkout_order_processed', function ( $order_id ) {  // LUVIT_CLUB_FLAG
	if ( ! LUVIT_CLUB_GIFT || ! function_exists( 'luvit_loyalty_state' ) ) {
		return;
	}
	...
	if ( $order->get_meta( '_luvit_club_flagged' ) ) {
		return;
	}

	/* الحالة بتنقرا **قبل** ما تنعدّ هالطلبية · فـ«due» يعني هاي هي. */
	$s = luvit_loyalty_state();
	if ( empty( $s['due'] ) ) {
		return;
	}
	...
	$order->add_order_note(
		'🎁 هدية النادي · هاي الطلبية رقم ' . (int) $s['next'] . ' لهالزبونة · حطّوا الهدية بالكرتونة.'
	);
	$order->update_meta_data( '_luvit_club_flagged', 1 );
	$order->save();
} , 20 );
...
add_action( 'init', function () {  // luvit_free_ship_settings
	/* ⚠️ **الاسم بيقول `free_ship` والمكافأة صارت هدية.**
	   ما بدّلنا الاسم بقصد: تبديله بيتطلّب ترحيل القيمة المخزّنة قبل
	   الإطلاق بسبعة أيام، والمكسب تسمية بس. **اقرا الاسم كـ«كل كم طلبية
	   تيجي مكافأة»** لا كوعد بتوصيل. [[moving-truth-breaks-its-readers]] */
...
/* ══════════════════════════════════════════════════════════════════════
   🎁 **المكافأة صارت هدية، مش توصيلاً مجانياً · قرار ريّان ٦ أيلول**
   ══════════════════════════════════════════════════════════════════════
   ليش انبدّلت · والسبب رقم لا ذوق:

   التوصيل المجاني **مركّب أصلاً** على أي روتين من أربع قطع (سنيبت 314).
   وأهم زبونة عنّا هي اللي بتشتري روتين. يعني جايزة «الطلبية الخامسة
   توصيلها مجاني» كانت **بتستبدل سطراً مجانياً بسطر مجاني تاني بعنوان
   مختلف** · قيمتها الفعلية لها **صفر**.

   ⤷ وتعليقنا إحنا كان بيقولها حرفياً: «فلو الاثنان انطبقوا الولاء بيغلب
     وبيعطي **نفس النتيجة (مجاني)**». كتبناها وما وقفنا عندها.

   ⚠️ **والقاعدة اللي بتطلع من هون وبتنطبق على أي مكافأة جاي:**
      مكافأة الزبونة **بتاخدها أصلاً** مش مكافأة.

   ── والهدية مفتوحة بقصد ─────────────────────────────────────────────
   ريّان: «خليها إشي عشوائي مع تنويه إنه حيكون إشي محترم».
   🔴 فالكوبي **ما بيسمّي نوع الهدية ولا قيمتها ولا بيوعد بصنف** · لأنّ
      اللي بيقرّر شو الهدية هو **صاحب العلامة** لا إحنا. [[ask-whose-decision-it-is]]

   ── والعدد صار ١٠ ───────────────────────────────────────────────────
   ⚠️ **وفيه ملاحظة مسجّلة على الرقم:** بدورة شراء عناية (شهرين لتلاتة)،
      عشر طلبات ≈ سنتين. وعدّاد بعيد بيوقف عن كونه تحفيز.
      **القرار قرار ريّان، والمتابعة بالبيانات:** منسجّل بعد الإطلاق كم
      يوم فعلياً بين الطلبية والتانية، والرقم بينراجع على أساس مقيس.
   ══════════════════════════════════════════════════════════════════════ */
if ( ! defined( 'LUVIT_CLUB_GIFT' ) ) {
	define( 'LUVIT_CLUB_GIFT', true );
}
...
add_filter( 'woocommerce_package_rates', function ( $rates, $package ) {  // LUVIT_FREE_SHIP
	/* 🔴 **مطفي من ٦ أيلول** · المكافأة صارت هدية بالكرتونة لا توصيلاً.
	   والكود بيضل كامل: لو رجعت المكافأة توصيلاً يوماً، شيل السطرين. */
	if ( LUVIT_CLUB_GIFT ) {
		return $rates;
	}
