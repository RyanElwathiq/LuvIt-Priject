...
/* 🔴 **حارس التحميل المزدوج · بيمنع شاشة بيضا على الموقع كله.**

   الملف بينركّب على مرحلتين: `plugins/` أول للفحص، وبعدين `mu-plugins/`
   للسرعة. وترويسة هالملف كانت بتقول «حطّها» ولا مرة قالت **«انقلها»**.
   ونسخة بالمكانين = **٢١ تعريفاً مكرّراً** = خطأ قاتل بـPHP، والموقع
   واللوحة الاتنين بيوقعوا · وما إله زرّ إطفاء لأنّ `mu-plugins` ما
   بتنعطّل.
   ⤷ ونفس الشي بيصير لو انرجّعت نسخة احتياطية فوق الحالية.

   ⚠️ **والحارس لازم يكون هون بالذات · قبل أول تعريف.**
      وممنوع يكون على `function_exists( 'luvit_d_router' )` · الملف
      **بيرجع من نصّه** لكل طلب مش `/d`، فحارس معلّق على دالة معرّفة
      بعد نقطة الرجوع بيكون ميتاً بالضبط على الطلبات اللي بتوقع الموقع. */
if ( defined( 'LUVIT_D_LOADED' ) ) {
	return;
}
define( 'LUVIT_D_LOADED', 1 );
